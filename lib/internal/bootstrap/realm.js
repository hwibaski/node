 ...
 */
// 변수선언
// c++ 코드 바인딩 하는 코드 같음.. 추정
let internalBinding;
{
  // bindingObj라는 상수를 선언하고, __proto__를 null로 설정한 빈 객체를 할당합니다.
  // 이 객체는 내부 바인딩을 캐시하는 데 사용됩니다.
  const bindingObj = { __proto__: null };
  // internalBinding이라는 함수를 정의하고, 이 함수를 internalBinding 변수에 할당합니다. module이라는 매개변수를 받습니다.
  // eslint-disable-next-line no-global-assign
  internalBinding = function internalBinding(module) {
    // bindingObj에서 module 이름을 키로 하여 변수 mod를 선언하고 초기화합니다.
    let mod = bindingObj[module];
    // mod가 객체가 아니라면 또는 null이 아니라면
    if (typeof mod !== 'object') {
      // getInternalBinding(module) 함수를 호출하여 반환된 값을 mod에 할당하고, 동시에 bindingObj[module]에도 할당합니다.
      mod = bindingObj[module] = getInternalBinding(module);
      
      // 잘은 모르겠지만 moduleLoadList에 Internal Binding ${module}을 push하는 코드 같음.
